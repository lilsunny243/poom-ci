...
public class RepositoryQuery<T> extends VertexQuery<T> {
    ...
    public Optional<T> repository(String repositoryId) {
        List<T> result = this.processTraversal(this.graph().V().hasLabel("repository").has("repository-id", repositoryId));
        return Optional.ofNullable(result.isEmpty() ? null : result.get(0));
    }

    public List<T> all() {
        return this.processTraversal(this.graph().V().hasLabel("repository"));
    }
}
